# update_pitchers.py  – full list, deduped, starter-only, four buckets, summaries
...
# ── 1. Paste/update your names here (duplicates OK, Wheeler OK) ─────────
NAME_LIST = """
Colin Rea
Paul Skenes
Seth Lugo
...
# ── 2. Deduplicate names & strip blanks ────────────────────────────────
    ...
    for full in sorted(names):
        try:
            ...
            if not lk.empty:
                ...
                print(f"Resolved {full:<25} → {mapping[full.replace(' ', '_')]}")
            else:
...
for name, pid in PITCHERS.items():
    ...
    # starter filter: faced first PA of his half-inning
